...
using Transport=std::function<PersonalCaptureSync::Response(const PersonalCaptureSync::Settings&,
    const std::wstring&,const std::wstring&,const std::string&,std::size_t,int)>;

class Library {
public:
    explicit Library(fs::path root,Transport transport={});
    Account account();
    void login(const std::wstring& server,const std::wstring& username,const std::wstring& password,
               const std::wstring& invitation=L"");
    void logout();
    std::vector<Record> list(const std::string& scope);
    Record save(const std::string& scope,Json record,const std::map<std::string,fs::path>& assets={},
...
private:
    ...
    PersonalCaptureSync::Response request(const Account& account,const std::wstring& method,
        const std::wstring& path,const std::string& payload={},std::size_t limit=8U*1024U*1024U,int revision=0);
};
}
