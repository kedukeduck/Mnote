# Mnote Windows 1.8.0-test

## 批量导出 Markdown

首页先按搜索、标签或类型筛选，点击“批量导出 Markdown”，按 Ctrl 多选 / Shift 连选（或全选），然后确认分享图片并选择 `.md` 保存位置。仅支持当前登录账号已同步的记录，每次 1–100 条；未同步、冲突、回收站及禁止 AI 访问的内容不会导出。
