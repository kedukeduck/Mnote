...
文件包含文档说明、AI 阅读提示、范围、索引、想法、摘录、完整保留的原文、来源和完整截图／圈选图／批注图在线链接。图片是独立快照，持有链接即可查看；在“设置 → 分享管理”撤销，不删除原记录。每账号最多保留 50 次有效导出，超出时请撤销旧分享。AI 软件需要联网并具备读图能力，不保证自动读取 Markdown 中的全部图片。
