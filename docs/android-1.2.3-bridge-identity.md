...
新增生命周期标题、实际窗口 ID 传递、改名 / 空根节点 / 无包名的已知过渡窗口、非本次 Mnote 窗口边界、来源包名缺失与身份冲突测试。

发布验证：修复前的生命周期测试在 API 30 / 35 均失败，修复后均通过；`scripts/verify-mnote-v1.sh` 全部通过，Android 261 项测试通过，lint 0 errors / 32 warnings；服务端含 MCP 的 20 项测试通过；Windows 构建与 GUI / 同步冒烟、浏览器扩展及 Manifest 检查通过。APK 测试签名与上一版一致。生产服务端未修改，没有手机实测结果。
