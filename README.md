...
当前 Windows 测试版 **1.11.0-test**：[安装包](https://chenyu.online/heartnote-capture/updates/files/mnote-windows-v1.11.0-test/Mnote-Windows-1.11.0-test-Setup.exe) · [便携包](https://chenyu.online/heartnote-capture/updates/files/mnote-windows-v1.11.0-test/Mnote-Windows-1.11.0-test-Portable.zip)。使用 Android 上同一账号登录，自动同步；旧本机记录需要明确导入。
...
当前 Android 增量测试版：[1.11.0-test APK](https://chenyu.online/heartnote-capture/updates/files/mnote-android-v1.11.0-test/Mnote-Android-1.11.0-test.apk)。分享管理直接显示历史图片预览卡片；记录与编辑时可搜索、多选已有标签，同时保留手写新标签。详见 [1.11.0 说明](docs/gallery-tags-1.11.0-release-notes.md)。安装前保存草稿，覆盖安装，不要卸载旧版。使用原调试/测试签名，非正式商店版。
